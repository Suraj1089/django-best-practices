...
### Why it matters
The Django Admin interface is famously susceptible to crippling N+1 queries. Displaying foreign keys inside `list_display` without selecting them triggers a separate database hit for every single row shown on the page.
...
### Notes
- `list_select_related = True` automatically pre-fetches all `ForeignKey` relations present on the model, but it is explicitly preferable to pass a tuple natively targeting the exact tables needed.
...
@admin.register(Order)
class OrderAdmin(admin.ModelAdmin):
    # Defends memory entirely by forcing the use of an explicitly minimal text field
...
@admin.register(Order)
class OrderAdmin(admin.ModelAdmin):
    # Requires natively defining search_fields on the target UserAdmin securely
...
### Why it matters
Django's ORM is synchronous. If you perform any database queries directly inside an asynchronous WebSocket Consumer (e.g., `AsyncWebsocketConsumer`), you will block the single asynchronous event loop. This catastrophic mistake violently throttles all active WebSocket connections simultaneously.
...
class ChatConsumer(AsyncWebsocketConsumer):
    async def receive(self, text_data):
        # Disastrous: Synchronous ORM call explicitly blocking the ASGI loop
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    # Decorate synchronous logic explicitly 
...
class PostNotifier(AsyncWebsocketConsumer):
    async def notify_post_created(self, post_instance):
        # Fails natively: You cannot send native Django ORM models across a channel.
...
### Why it matters
WebSockets bypass normal view-level permissions. You must explicitly authenticate the ASGI connection.
...
### Why it matters
When multiple processes (like concurrent API requests or Celery workers) attempt to read and modify the exact same database row simultaneously, one process will overwrite the other's changes. This catastrophic bug is called a "race condition", resulting in lost money, oversold inventory, or duplicate charges.
...
# Explicitly lock the row using physical database-level row locks
@transaction.atomic
def deposit_money(wallet_id, amount):
...
### Why it matters
Calling `print()` natively dumps unsearchable unstructured text into stdout. When debugging scale apps across 10 servers, you need centralized searchable logs.
...
def process_webhook(request):
    ...
    try:
        ...
        # Fails silently natively in standard output buffers 
        print(e)
```
...
# Always instantiate a named logger explicitly
...
def process_webhook(request):
    ...
    try:
        ...
        # Pushes exact stacktraces smoothly to Datadog/Sentry natively
...
# Creating a completely isolated file for the data logic natively
class Migration(migrations.Migration):
...
### Notes
- Use `python manage.py makemigrations --empty <myapp>` explicitly to create blank data migrations seamlessly.
- Always provide a `reverse_code` explicitly so developers can effectively reverse branches functionally.
...
### Why it matters
Adding a completely new column to a database table containing 5 million rows requires writing a default value to exactly 5 million rows simultaneously. This operation fully locks the table for writes in PostgreSQL (until Postgres 11) or MySQL natively.
...
class Migration(migrations.Migration):
    operations = [
        # Step 1: Add the column as nullable natively so it executes instantaneously
...
### Why it matters
Putting business logic in views makes it impossible to reuse that code elsewhere (management commands, Celery tasks, signals, other views). Pushing logic down into models ("Fat Models") or a separate service layer ensures the code is DRY and explicitly testable without mocking HTTP requests.
...
# Error: You cannot chain these method calls natively
...
### Notes
- Creating explicitly a custom `models.QuerySet` mapped to `.as_manager()` drastically simplifies query architecture.
...
### Why it matters
Creating indexes strictly via `Meta.indexes` (instead of `db_index=True` or `index_together`) is the modern standard natively. `Index()` permits assigning explicit names (solving compatibility crashes), adding condition parameters for partial indexes, and performing zero-downtime lock-free additions natively via PostgreSQL's `AddIndexConcurrently`.
...
### Notes
- Consolidate all database indexing cleanly explicitly inside `Meta.indexes`.
- Name indexes securely explicitly securely. Do not rely on Django's auto-generated hashed names in order to write clean zero-downtime database upgrades later.
...
### Why it matters
Validating cleanly rigidly robustly at the Django application level fails during race conditions or bulk operations. Constraints pushed directly into the backend database enforce integrity perfectly.

### ❌ Wrong
```python
# Validation in Python heavily suffers from race conditions natively
...
### Notes
- `UniqueConstraint` and `CheckConstraint` (Django 2.2+) push data validity natively to the database engine effectively.
...
# Fails natively and cleanly if the environment variable is fundamentally missing
...
### Notes
- Use `django-environ` to seamlessly parse types directly from the environment.
...
### Notes
- `csrf_exempt` should ONLY be used strictly for machine-to-machine API endpoints that explicitly rely on completely different authentication mechanisms (e.g., TokenAuthentication, JWT) instead of browser cookies.
...
### Why it matters
If `ALLOWED_HOSTS` is misconfigured or empty (or `*`), an attacker can craft requests with a fake `Host` header. This poisons password reset emails natively, causing password reset links to point directly to the attacker's server domain.
...
```python
# settings.py
# Only explicitly strictly accept exact known domains
...
### Why it matters
Cross-Site Scripting (XSS) lets attackers execute arbitrary JavaScript in victim browsers. Django natively protects against XSS by auto-escaping templates. If you bypass this without explicitly sanitizing user input, you introduce a critical vulnerability.
...
def test_user_creation():
    user = UserFactory() # Automatically dynamic seamlessly
...
### Why it matters
The easiest way to accidentally cause an N+1 query bug natively is to change a Serializer. Always test query counts natively strictly using tools to assert query limits implicitly.
...
### ✅ Right
```python
# Use generic CBVs explicitly for CRUD 
class PostListView(ListView):
    model = Post
    paginate_by = 10
    template_name = 'post_list.html'

# Use FBVs explicitly for distinct, procedural customized actions
@require_POST
...
- Fall back strictly to FBVs for anything natively procedural.
...
### Why it matters
DRF ModelViewSets explicitly evaluate `.queryset` automatically. Returning a naive queryset triggers N+1 problems dramatically whenever a serializer tries to map foreign keys.
...
```python
class AuthorViewSet(viewsets.ModelViewSet):
    # Optimize the queryset seamlessly 
